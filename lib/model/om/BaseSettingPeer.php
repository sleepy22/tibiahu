...
/**
 * Base static class for performing query and update operations on the 'tibia_setting' table.
 *
 * 
 *
...
abstract class BaseSettingPeer {
	...
	const TABLE_NAME = 'tibia_setting';

	/** A class that can be returned by this peer. */
	const CLASS_DEFAULT = 'lib.model.Setting';
	...
	const KEY = 'tibia_setting.KEY';

	/** the column name for the VALUE field */
	const VALUE = 'tibia_setting.VALUE';
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(SettingPeer::KEY);
    ...
    foreach (sfMixer::getCallables('BaseSettingPeer:doCount:doCount') as $callable)
    {
      call_user_func($callable, 'BaseSettingPeer', $criteria, $con);
    }
    ...
    foreach (sfMixer::getCallables('BaseSettingPeer:doSelectStmt:doSelectStmt') as $callable)
    {
      call_user_func($callable, 'BaseSettingPeer', $criteria, $con);
    }
  ...
  static public function getUniqueColumnNames()
  {
    ...
    foreach (sfMixer::getCallables('BaseSettingPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseSettingPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
    ...
    foreach (sfMixer::getCallables('BaseSettingPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseSettingPeer', $values, $con, $pk);
    }
    ...
    foreach (sfMixer::getCallables('BaseSettingPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseSettingPeer', $values, $con);
      if (false !== $ret)
      {
		...
		if ($values instanceof Criteria) {
			...
			$comparison = $criteria->getComparison(SettingPeer::KEY);
			$selectCriteria->add(SettingPeer::KEY, $criteria->remove(SettingPeer::KEY), $comparison);
    ...
    foreach (sfMixer::getCallables('BaseSettingPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseSettingPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the tibia_setting table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		if ($values instanceof Criteria) {
			...
			$criteria->add(SettingPeer::KEY, (array) $values, Criteria::IN);
	...
	public static function doValidate(Setting $obj, $cols = null)
	{
		...
		$res =  BasePeer::doValidate(SettingPeer::DATABASE_NAME, SettingPeer::TABLE_NAME, $columns);
    if ($res !== true) {
        $request = sfContext::getInstance()->getRequest();
        foreach ($res as $failed) {
            $col = SettingPeer::translateFieldname($failed->getColumn(), BasePeer::TYPE_COLNAME, BasePeer::TYPE_PHPNAME);
        }
    }
	...
	public static function retrieveByPK($pk, PropelPDO $con = null)
	{
		...
		$criteria->add(SettingPeer::KEY, $pk);
	...
	public static function retrieveByPKs($pks, PropelPDO $con = null)
	{
		...
		if (empty($pks)) {
			...
			$criteria = new Criteria(SettingPeer::DATABASE_NAME);
			$criteria->add(SettingPeer::KEY, $pks, Criteria::IN);
