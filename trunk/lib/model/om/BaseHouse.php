...
/**
 * Base class that represents a row from the 'tibia_house' table.
 *
 * 
 *
...
abstract class BaseHouse extends BaseObject  implements Persistent {
  ...
  const PEER = 'HousePeer';
	...
	public function setSlug($v)
	{
		...
		if ($this->slug !== $v) {
			$this->slug = $v;
			$this->modifiedColumns[] = HousePeer::SLUG;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = HousePeer::NUM_COLUMNS - HousePeer::NUM_LAZY_LOAD_COLUMNS).
    ...
    foreach (sfMixer::getCallables('BaseHouse:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseHouse:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseHouse:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseHouse:save:post') as $callable)
    {
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = HousePeer::doInsert($this, $con);
					...
					$affectedRows += HousePeer::doUpdate($this, $con);
				}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(HousePeer::SLUG)) $criteria->add(HousePeer::SLUG, $this->slug);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(HousePeer::SLUG, $this->slug);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseHouse:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseHouse::%s', $method));
    }
